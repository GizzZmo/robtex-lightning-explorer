...
app.get('/health', (_req, res) => {
  res.json({ ok: true, service: 'robtex-lightning-explorer' });
});
...
app.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const offset = Number(req.query.offset) || 0;
    const data = await client.addressTransactions(req.params.address, limit, offset);
    res.json(data);
  }),
);

app.listen(PORT, () => {
  console.log(`⚡ Robtex LN Explorer running at http://localhost:${PORT}`);
